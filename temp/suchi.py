...
def copy_to_x11(text, selection='clipboard'):
    try:
        # xclip forks and detaches on its own once it owns the selection, but we
        # still start a new session so it survives the terminal closing instantly.
        ...
        process.stdin.close()  # Close stdin to signal EOF to xclip
    except Exception:
        pass

def load_all(path):
    """Load the full history file, unbounded (unlike validate_and_load, which trims to `limit`)."""
    if not os.path.exists(path):
        return []
    try:
        with open(path, 'r', encoding='utf-8') as f:
...
def touch_used(path, text):
    """Mark an item as just-used so it sorts to the top next time (matches
    clip_daemon_x11.py's schema: sort key is max(usedAt, copiedAt))."""
    data = load_all(path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("file", nargs="?", default=os.path.expanduser("~/.cache/suchi/history.json"))
